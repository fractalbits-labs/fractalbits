...
use volume_group_proxy::{AtOrBeforeRead, DataVgProxy};
...
impl StorageBackend {
    ...
    /// At-or-before block read: the newest generation at or below the
    /// layout's committed ceiling. The selected identity/classification
    /// comes back with the bytes.
    pub async fn read_block_at_or_before(
        &self,
    ...
    /// Write a punch/trim tombstone generation at `version`: an
    /// authoritative hole for at-or-before reads.
    pub async fn write_tombstone_block(
        &self,
        blob_guid: DataBlobGuid,
        block_number: u32,
        version: u64,
        ...
        self.data_vg_proxy
            .put_blob_tombstone(blob_guid, block_number, version, trace_id)
            .await?;
        Ok(())
    }
