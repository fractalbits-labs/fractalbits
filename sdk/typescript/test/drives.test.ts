...
import { ApiError, FractalbitsClient, normalizeSubPath } from "../src/index.js";
...
describe("mountConfig", () => {
  it("renders the fractalbits-mount config", () => {
  ...
  it("validates subPath", () => {
    expect(normalizeSubPath(undefined)).toBe("/");
    expect(normalizeSubPath("/a/b/")).toBe("/a/b/");
    expect(() => normalizeSubPath("a/")).toThrow();
    expect(() => normalizeSubPath("/a")).toThrow();
    expect(() => normalizeSubPath("/a//b/")).toThrow();
    expect(() => normalizeSubPath("/../")).toThrow();
  });
});
