...
impl DataVgProxy {
    ...
    pub fn new_with_circuit_breaker(
        ...
        Ok(Self {
            ...
            ec_hedge_delay: DEFAULT_EC_HEDGE_DELAY,
        })
    }

    /// Override the EC read hedge delay (defaults to
    /// [`DEFAULT_EC_HEDGE_DELAY`]).
    pub fn with_ec_hedge_delay(mut self, delay: Duration) -> Self {
        self.ec_hedge_delay = delay;
        self
    }
    ...
    async fn get_blob_from_node_instance(
        ...
        let _result_label = match &result {
            Ok(_) => "success",
            Err(RpcError::NotFound) => "not_found",
            Err(_) => "failure",
        };
            ...
            match result {
                Ok(node_entries) => {
                    ...
                    for entry in node_entries {
                        let key = (entry.block_number, entry.version);
    ...
    async fn get_blob_ec(
        ...
        let mut hedge_delay = Box::pin(rpc_client_common::rpc_sleep(self.ec_hedge_delay));
