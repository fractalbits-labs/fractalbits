...
use file_ops::{
    ListEntry, create_dir_marker_layout, mpu_get_part_prefix, parse_delete_inode,
...
impl StorageBackend {
    ...
    pub async fn get_inode(&self, key: &str, trace_id: &TraceId) -> Result<ObjectLayout, FsError> {
        self.get_inode_with_bytes(key, trace_id)
            .await
            .map(|(layout, _)| layout)
    }
    ...
    pub async fn fence_data_write_token(
        ...
        self.data_vg_proxy
            .fence_data_write_token(blob_guid, version, write_token, trace_id)
            .await?;
        Ok(())
    }
    ...
    pub fn enqueue_superseded_generation_gc(
        ...
        self.data_vg_proxy.enqueue_superseded_generation_gc(
            blob_guid,
            block_numbers,
            keep_version,
            trace_id,
    ...
    pub async fn list_blob_blocks_at_or_before(
        ...
        committed_token: u64,
        trace_id: &TraceId,
    ) -> Result<std::collections::BTreeSet<u32>, FsError> {
        Ok(self
            .data_vg_proxy
    ...
    pub async fn get_inode_record(
        ...
        self.get_inode_record_with_bytes(inode_id, trace_id)
            .await
            .map(|(record, _)| record)
    }

    pub async fn get_inode_record_with_bytes(
        &self,
        inode_id: uuid::Uuid,
        trace_id: &TraceId,
    ) -> Result<(InodeRecord, Bytes), FsError> {
    ...
    pub async fn delete_blob_blocks(
        ...
        // Fence only a pending prepare lease: its writer may still be
        // streaming blocks for minutes and must be stopped before the sweep.
        // The committed token is not fenced; the only writes that can trail
        // the sweep are background quorum completions of already-acked puts
        // (a seconds-wide window), and a fragment they resurrect is invisible
        // garbage on a never-reused blob_guid. A permanent per-delete fence
        // record on every node would be the worse leak.
        if let Some(pending) = layout.pending_data_write {
            self.fence_data_write_token(blob_guid, pending.version, pending.token, trace_id)
                .await?;
        }
        self.data_vg_proxy
