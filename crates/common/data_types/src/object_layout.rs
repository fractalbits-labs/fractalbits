...
pub struct ObjectLayout {
    ...
    pub block_size: u32,
    /// Burned-version watermark: the highest completed or recovered data
    /// generation for this blob. An in-progress generation is durably held
    /// in `prepared_write` before any fragment is written.
...
pub struct FsExt {
    ...
    /// only created or appended, without holes or reservations). Only
    /// meaningful for `ObjectState::Normal`.
    pub block_map: Option<BlockMapRef>,
    /// Crash-recovery state for the one supported writer. A flush installs
    /// this before data I/O and clears it on commit. The next data flush
    /// immediately makes an interrupted attempt's exact keys unreachable.
    /// Boxed: it exists only between a prepare CAS and its commit or
    /// recovery, so steady-state rows keep only the `None` niche instead
    /// of ~48 reserved bytes.
    pub prepared_write: Option<Box<PreparedWrite>>,
}
...
/// Minimal recovery record for an interrupted single-writer flush.
#[derive(Debug, Archive, Deserialize, Serialize, PartialEq, Eq, Clone, Copy)]
pub struct PreparedWrite {
    /// The generation burned by the prepare CAS.
    pub version: u64,
    /// Inclusive version-1 append span to turn into holes on recovery.
    pub append_range: Option<(u32, u32)>,
    /// Pre-published map that hides any converted reservations on recovery.
    pub reservation_abort_map: Option<BlockMapRef>,
}
...
impl ObjectLayout {
    ...
    #[inline]
    pub fn block_map(&self) -> Option<BlockMapRef> {
        self.fs_ext.as_ref().and_then(|e| e.block_map)
    }

    #[inline]
    pub fn prepared_write(&self) -> Option<PreparedWrite> {
        self.fs_ext
            .as_ref()
            .and_then(|e| e.prepared_write.as_deref().copied())
    }
    ...
    pub fn set_block_map(&mut self, block_map: Option<BlockMapRef>) {
        self.update_fs_ext(|e| e.block_map = block_map);
    }

    pub fn set_prepared_write(&mut self, prepared_write: Option<PreparedWrite>) {
        self.update_fs_ext(|e| e.prepared_write = prepared_write.map(Box::new));
    }

    /// Normalized constructor for layout literals: `None` when every
    /// member is `None`.
    pub fn fs_ext_from(
        posix: Option<PosixAttrs>,
        block_map: Option<BlockMapRef>,
        prepared_write: Option<PreparedWrite>,
    ) -> Option<Box<FsExt>> {
        let ext = FsExt {
            posix,
            block_map,
            prepared_write: prepared_write.map(Box::new),
        };
...
mod tests {
    ...
    fn fs_ext_round_trips_and_normalizes() {
        ...
        multi.set_block_map(Some(BlockMapRef {
            map_id: Uuid::nil(),
            chunk_count: 2,
        }));
        multi.set_prepared_write(Some(PreparedWrite {
            version: 3,
            append_range: None,
            reservation_abort_map: None,
        }));
        multi.set_fs_posix(Some(p));
        assert_eq!(multi.block_map().map(|m| m.chunk_count), Some(2));
        assert_eq!(multi.prepared_write().map(|w| w.version), Some(3));
        assert_eq!(multi.fs_posix(), Some(p));
        multi.set_prepared_write(None);
        assert!(multi.prepared_write().is_none());
        assert_eq!(multi.block_map().map(|m| m.chunk_count), Some(2));
    }
