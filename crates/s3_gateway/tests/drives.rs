...
async fn test_rss_fenced_deletes() {
    ...
    let t = TraceId::new();
    let key = "drive:fence-test";
    let _ = rss.delete(key, 0, None, &t, 0).await;

    // Read version N, replace the record (N+1), delete with N: refused,
    // and the replacement stays.
    let written = rss.put(0, key, "v1", None, &t, 0).await.expect("create");
    let (n, v1) = rss.get(key, None, &t, 0).await.expect("get v1");
    assert_eq!(v1, "v1", "first value");
    assert_eq!(written, n, "put returns the version get reports");
    let written = rss.put(n, key, "v2", None, &t, 0).await.expect("replace");
    let (n2, v2) = rss.get(key, None, &t, 0).await.expect("get v2");
    assert_eq!(v2, "v2", "replaced value");
    assert_eq!(written, n2, "replacement's version comes back from put");
    let stale = rss.delete(key, n, None, &t, 0).await;
    assert!(
        matches!(stale, Err(RpcError::Retry)),
        "stale delete: {stale:?}"
    );
    let (_, still) = rss.get(key, None, &t, 0).await.expect("still there");
    assert_eq!(still, "v2", "replacement untouched");
    rss.delete(key, n2, None, &t, 0)
        .await
        .expect("current delete");
    let gone = rss.get(key, None, &t, 0).await;
    ...
    ctx.create_bucket(name).await;
    let (_, bucket) = rss
        .get(&format!("bucket:{name}"), None, &t, 0)
        .await
    ...
    let wrong = rss
        .delete_bucket(name, TEST_KEY, "not-this-root", None, &t, 0)
        .await;
    assert!(
    ...
    rss.delete_bucket(name, TEST_KEY, &root, None, &t, 0)
        .await
